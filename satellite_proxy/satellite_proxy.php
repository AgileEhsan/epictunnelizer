...
// GNU General Public License for more details.
	...
	if ($b) {
		...
		header( "WWW-Authenticate: Basic realm=\"EpicTunnelizer\"" );
	...
	if ($_REQUEST["a"]=="c") {
		...
		if (!($copts & 2) && $ENCRYPTION_FORCE) {
			echo "c:s=ER&msg=Proxy+server+does+not+support+unencrypted+connections\n"; exit;}
	    ...
	    logline (2,$copts & 4?
    	"$ident: New connection established ".$_SERVER["REMOTE_ADDR"]." sending UDP packets":
    	($copts & 8?
    	"$ident: New connection established ".$_SERVER["REMOTE_ADDR"]." listening on port $bpo":
    	"$ident: New connection established ".$_SERVER["REMOTE_ADDR"]." -> $dad:$dpo"));
